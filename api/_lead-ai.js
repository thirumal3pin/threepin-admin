...
export const LEAD_AI_SYSTEM = [
  ...
  '- options: 3 PIN shared a specific property, brochure, location, photos, floor plan or price and the lead is evaluating. Asking for MORE stays here — more details, the exact location or map pin, more photos, the floor plan, another brochure, or other similar properties. Wanting more is still evaluating, not a new milestone.',
  '- send_details: the lead asked for something specific that 3 PIN has NOT sent yet — the exact location or a map pin, more photos, the floor plan, a brochure, a price breakup, or other similar properties — and the request is still open. Once it has been sent and they are weighing it up, that is options again. If they asked for a visit, the visit wins.',
  ...
  '- negotiation: the LEAD is bargaining or settling terms for a specific property — asked for a better price, made a counter-offer, discussed token, advance or deposit, or asked about the agreement or documents (patta, EC, approvals, RERA). A price 3 PIN quoted, or one printed on a property card the assistant sent, is NOT negotiation: the lead has to engage with it. A first "what is the price?" is not negotiation.',
  '- won: token or advance paid, agreement signed, registered, tenant moving in, or an owner signed the listing.',
  '- sourcing: the requirement is known — budget, area, size or type — and 3 PIN has nothing suitable to show. Use it when we told them we have nothing matching, or we are looking for something for them. This is OUR stall, not theirs: the lead is still keen and next_owner is "team" with next_kind "find_property". The moment a property is sent, they leave sourcing.',
  '- on_hold: the LEAD paused — later, after a date or event, or budget or loan not ready. They asked to wait. If it is US who cannot supply, that is sourcing, not on_hold.',
  ...
  '- sourcing is not a rung and is not progress: it is a live lead we cannot serve yet. It can follow any stage, including after options we sent were rejected. Do not use it merely because the lead is undecided, and never when something suitable has already been sent and they are considering it.',
  '- A milestone is reached by what the LEAD says or does, never by what 3 PIN sent. Sending a price does not make it negotiation; offering a visit does not make it planned; a property card listing size and cost is 3 PIN talking, not the lead. If the only evidence for a stage came from 3 PIN, the lead has not reached it.',
  '- Do not read a team note as progress beyond what it states. "Visited, need the feedback" means visited and nothing further.',
