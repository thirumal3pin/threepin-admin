...
try {
  ...
  const first = await page.evaluate(() => ({ search: !!document.getElementById('actFind'), tiles: [...document.querySelectorAll('#chooser .cat b')].map(b => b.textContent.trim()), cards: document.querySelectorAll('#chooser .act').length }));
  check('Seven categories come first, and no actions until one is chosen', first.search && first.tiles.length === 7 && first.tiles[0] === 'Money in' && first.tiles[1] === 'Money out' && first.cards === 0, JSON.stringify(first));
  await page.click('#chooser .cat[data-g="money-out"]');
  await page.waitForTimeout(300);
  const onlyOne = await page.evaluate(() => ({ groups: document.querySelectorAll('#chooser .rec-group').length, cards: document.querySelectorAll('#chooser .act').length, back: !!document.querySelector('#chooser .cat-head .btn') }));
  check('Choosing a category shows its actions, with a way back', onlyOne.groups === 1 && onlyOne.cards >= 10 && onlyOne.back, JSON.stringify(onlyOne));
  await page.evaluate(() => window.fin.pickGroup(''));
  await page.fill('#actFind', 'emi');
  await page.waitForTimeout(300);
  const hits = await page.evaluate(() => [...document.querySelectorAll('#chooser .act b')].map(b => b.textContent.trim()));
  check('Search finds by a word', hits.length >= 2 && hits.some(h => /EMI/i.test(h)), JSON.stringify(hits));
  await page.fill('#actFind', '');
  await page.evaluate(() => window.fin.pickGroup('money-in'));
  await page.waitForTimeout(300);
  const inCue = await page.evaluate(() => { const b = document.querySelector('#chooser .act.dir-in'); return { n: document.querySelectorAll('#chooser .act.dir-in').length, c: b ? getComputedStyle(b, '::before').backgroundColor : '' }; });
  await page.evaluate(() => window.fin.pickGroup('money-out'));
  await page.waitForTimeout(300);
  const cues = await page.evaluate(() => {
    const c = sel => { const b = document.querySelector(sel); return b ? getComputedStyle(b, '::before').backgroundColor : ''; };
    return { outN: document.querySelectorAll('#chooser .act.dir-out').length, outC: c('#chooser .act.dir-out') };
  });
  check('Money-in cards carry a green rule', inCue.n >= 3 && inCue.c === 'rgb(31, 122, 77)', JSON.stringify(inCue));
  check('Money-out cards carry a rust rule', cues.outN >= 5 && cues.outC === 'rgb(154, 74, 44)', JSON.stringify(cues));
