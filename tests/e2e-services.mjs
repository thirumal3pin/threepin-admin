...
try {
  ...
  const first = await page.evaluate(() => ({ search: !!document.getElementById('actFind'), chips: document.querySelectorAll('#recGroups .chip').length, usual: document.querySelectorAll('#recUsual .act').length, cards: document.querySelectorAll('#chooser .act').length, folds: document.querySelectorAll('#chooser .rec-more').length }));
  check('Search, six group chips and "your usual" come first', first.search && first.chips === 7 && first.usual >= 4, JSON.stringify(first));
  check('Everyday actions are open; rarer ones are folded by name', first.cards >= 12 && first.folds >= 3, JSON.stringify(first));
  await page.click('#recGroups .chip[data-g="expenses-bills"]');
  await page.waitForTimeout(300);
  const onlyOne = await page.evaluate(() => ({ groups: document.querySelectorAll('#chooser .rec-group').length, cards: document.querySelectorAll('#chooser .act').length }));
  check('Choosing a group shows that group alone, every tier', onlyOne.groups === 1 && onlyOne.cards >= 10, JSON.stringify(onlyOne));
  await page.click('#recGroups .chip[data-g=""]');
  ...
  const cues = await page.evaluate(() => {
    ...
    return { inN: document.querySelectorAll('#chooser .act.dir-in').length, outN: document.querySelectorAll('#chooser .act.dir-out').length, inC: c('#chooser .act.dir-in'), outC: c('#chooser .act.dir-out') };
  });
  check('Money-in cards carry a green rule', cues.inN >= 3 && cues.inC === 'rgb(31, 122, 77)', JSON.stringify(cues));
