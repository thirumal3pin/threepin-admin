...
function renderConversationReply(l){
  ...
  el.innerHTML = `
    <a class="dp-reply-btn" href="${escapeHtml(url)}" target="${TT_TAB}" rel="noopener">↪ Reply in TailorTalk</a>
    <div class="dp-reply-note">${closed
      ? 'Opens in a tab, already on this chat. The 24-hour window has closed, so TailorTalk will need a template.'
      : 'Opens in a tab, already on this chat. TailorTalk does not allow its dashboard inside another page, so the reply is typed there.'}</div>`;
}
...
function openDetail(id){
  ...
  if(ttBtn){
    ...
    ttBtn.hidden = !ttUrl;
    if(ttUrl){ ttBtn.href = ttUrl; ttBtn.target = TT_TAB; }
  }
...
// ═══════ WHY THIS IS NOT A PANEL IN THE PAGE ═══════
//
// The obvious build is to embed the TailorTalk page here. It cannot be done:
// every response from dashboard.tailortalk.ai carries
//
//     x-frame-options: DENY
//     content-security-policy: frame-ancestors 'none'
//
// which instructs the browser that no site may frame it, enforced before any
// of our code runs. Their session cookie is SameSite=lax too, so even a
// permitted frame would show a login screen. The only way round it is
// proxying their dashboard through our own domain and rewriting those
// headers, which would put their login through our server and break on each
// of their deploys.
//
// What the CRM does instead is hold the conversation itself — see ttMsgBody,
// which renders the photos, brochures, voice-note transcriptions and shared
// locations that used to be the reason to go and look. Only TYPING a reply
// needs their dashboard, because the free-form send endpoint wants a
// dashboard login rather than the API token this CRM holds.
//
// TT_TAB is a named target, so the handover opens a TAB and every later lead
// reuses that same tab instead of leaving a row of them behind.
const TT_TAB = 'tailortalkChat';
